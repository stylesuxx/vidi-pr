...
- Each finding stays one single-line bullet starting with a bold severity tag (`**[high]**`, `**[medium]**`, or `**[low]**`), then plain prose, then `; fix: ...`. Never expand a finding into a nested "Fix:" sub-bullet. Severity tags go only on Findings, never on Suggestions or Positives.
